package lims.androidsensor.server;
		...
		listenThread = new Thread() {
			public void run() {
				...
				try {
					while (keepAlive) {
						try {
							// Try to accept a new connection on a specific port
							// If a new connection is not available, throw IOException
							// In IOIOLib, this call blocks the program in a synchronized block
							// but here it's run in a thread
							...
							// calls 'close' on the serverSocket object. 
							// This means we should break out of the connection accept loop.
							keepAlive = false;
						}
					}
				} catch (IOException e) {
